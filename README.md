...
- **Pluggable tax profiles** — Settings → Tax & Cost Basis selects your jurisdiction, which sets the full tax profile: tax-year boundary, allowance/exemption, rates, cost-basis method, holding-period rules, and the currency your tax figures are reported in. **Ten profiles** (UK is free; the rest are Vault Pro):
  ...
  - **Canada** — CAD, calendar year, ACB pooling, **50% inclusion rate** (only half a net capital gain is taxable). Skins held for investment are treated as **ordinary capital property** (v3.6.1): real cost and proceeds are used (no $1,000 personal-use-property floor) and **capital losses are deductible** against gains in the normal way. The alternative PUP reading ($1,000 floor + denied losses + LPP ring-fencing) and the business-income risk for habitual flipping are disclosed in the disclaimer, not modelled
