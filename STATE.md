...
Last updated: June 2026 | Current version: v3.2.0
...
- ⚠ **Re-verify caveat (carry into launch month):** all tax figures/thresholds across ALL 10 jurisdictions were implemented from secondary sources (Cowork `05-tax-rules.md` for UK/US/DE/CA; 2026 web tax references for SE/PL/AU/NO/DK/FI), none of which should be shipped on alone. Before charging anyone, re-check every rate/threshold/rule against **primary government pages**: HMRC (UK), IRS (US), BZSt (DE), CRA (CA), Skatteverket (SE), the Polish KAS (PL), the ATO (AU — incl. the 2027 discount→indexation reform), Skatteetaten (NO — incl. the share-vs-general-asset classification), Skattestyrelsen (DK — incl. speculative-intent + marginal rate), and Vero.fi (FI — incl. the €1,000 proceeds exemption vs the app's gains-cliff approximation). Allowances/bands change yearly
...
- **Vault Pro Phase 4b — Payments, Licensing, Code Signing** (next, target v3.1.x / v3.2.x — BLOCKED on external admin Rudi hasn't started yet): 
  - **Licence model decided: Paddle-native validation** (chosen for cheapest + least maintenance for a solo, non-code-literate dev — Paddle itself tracks who paid; no self-hosted licence server to babysit). `isPro()` from 4a plugs straight into it
  - **Webhook hosting decided: Cloudflare Workers** (free tier, 100k req/day — far more than a solo app needs; Rudi already uses Cloudflare for DNS, though Workers is a separate product). Receives Paddle webhooks, answers the "did they pay?" check
  - Still to build in 4b: Paddle checkout integration, licence validation + ~14-day offline grace, 14-day no-card Pro trial, OV/EV **code signing** wired into GitHub Actions (EV now needs cloud HSM signing e.g. Azure Trusted Signing), privacy policy + ToS pages
  - **External prerequisites Rudi must obtain first**: Paddle account (approval), code-signing cert (~£200–400/yr), and the pricing decision (Rudi is having Cowork research the market before deciding — make price a config constant, do not hardcode). Benchmark: SkinKeeper Pro ~$4.99/mo / $34.99/yr
...
- **Canada LPP loss ring-fencing (deeper than v3.1.1)** — v3.1.1 discloses but does not model it (the app pools all gains/losses). A future pass could add an LPP tag on items so LPP losses only offset LPP gains. Needs a reliable LPP signal first (art/jewellery/rare collectibles) — low priority until a CA customer asks
...
- **`TAX_PROFILES`** map — **10 jurisdictions as of v3.2.0**: UK, US, DE, CA, SE, PL, AU, NO, DK, FI. Selected by `getTaxJurisdiction()` → `getActiveTaxProfile()`. Each profile is a flat object: `code`, `name`, `taxCurrency`, `taxYearStart(now)`, `taxYearLabel(now)`, `allowance`, `allowanceIsCliff?` (DE, FI), `pupFloor?` (CA), `perDisposalInclusion?` (AU), `rates` (flat / banded / two-tier), `inclusionRate?` (CA), `feeDeductible`, `disposalCounts(trade)`, `classifyGain(disposal)`, `disclaimer`, `knownLimits?`
...
- **FI two-tier rate (v3.2.0)** — `rates: { lower, upper, threshold }`; tax = `min(taxable, threshold)*lower% + max(0, taxable-threshold)*upper%`, wired into all three estimate sites. FI also reuses the `allowanceIsCliff` machinery for its €1,000 small-sales exemption (approximated as a gains cliff)
...
- **`_applyExemption(netGain, profile)`** — the single source of truth for "gain remaining taxable after the annual exemption". Two regimes: **cliff** (`allowanceIsCliff`, DE Freigrenze — returns 0 below the threshold, the FULL gain at/above it) vs **deductible** (UK allowance — returns `max(0, gain − allowance)`). `inclusionRate` (CA 50%) is applied by the caller AFTER, on the remainder
...
- **Offline harness** `test-tax-engine.js` extracts the real `TAX_PROFILES` + helpers from `src/app.js` and asserts 36 boundary cases (see What's Been Built → v3.1.1). Run: `node test-tax-engine.js`
