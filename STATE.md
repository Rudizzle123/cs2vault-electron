...
- **TAX FIGURES NOT YET PRIMARY-SOURCE VERIFIED.** All 10 jurisdictions' rates/thresholds were implemented from secondary sources. Now that v3.3.0 wires up real payments, this is the last correctness gate before charging anyone. Do NOT take a paid sale until every figure is re-checked against primary government pages in the launch month (see the carried-forward caveat below). The app's disclaimers state "estimates, not tax advice", but charging for wrong figures is still a reputational/liability risk
...
- **🚦 LAUNCH GATE — before the first paid sale (all external admin Rudi must do; code is built to the boundary):**
  1. **Re-verify ALL 10 jurisdictions' tax figures against primary government pages** (HMRC/IRS/BZSt/CRA/Skatteverket/KAS/ATO/Skatteetaten/Skattestyrelsen/Vero.fi) in the launch month — this is the #1 correctness gate now that payments work
  ...
  6. **Fill the `[ACTION REQUIRED]` legal-name + governing-law markers** in `legal/privacy.html` and `legal/terms.html`; have both reviewed for the UK before taking payments
  7. **Business structure / liability** (sole trader vs ltd) — worth sorting before taking payments
  8. End-to-end **sandbox test**: Paddle sandbox purchase → Worker stores the licence → paste key → app flips to PRO → pull network → relaunch → Pro stays unlocked (offline grace)
