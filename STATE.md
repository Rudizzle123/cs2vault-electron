...
- **🚦 LAUNCH GATE — before the first paid sale (all external admin Rudi must do; code is built to the boundary):**
  ...
  9. **⚖ DECIDE — repo visibility before the first paid sale.** The code repo is currently public, which exposes the full tax engine (the paid product) and the `isPro()` gate — anyone can clone, flip `isPro()` to `return true`, and build an unlocked copy; a shared "free Pro" build in CS2 trading communities is the realistic risk. Recommended: **go private + create a separate public releases-only repo** (e.g. `cs2vault-releases`) because private-repo release assets require auth. That switch touches three things: (a) the website "Download Free" buttons (`releases/latest` links across all `website/` pages), (b) electron-updater's `publish` config in `package.json` — auto-update feeds must point at the public releases repo or updates break for existing installs, (c) the GitHub Actions release workflow target. Counter-argument on record: Electron apps are trivially unpackable anyway, and "open source, local-first — audit it yourself" is real marketing for an app handling financial data and (Phase 6) Steam credentials; if staying public, add a security page making that case deliberately. Phase 6's `safeStorage` credential-handling code strengthens the case for private. Decision is Rudi's; implementation is a normal versioned session once decided
