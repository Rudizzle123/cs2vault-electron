...
Last updated: July 2026 (v3.8.0: Phase 6 session 2 — GC item schema mapping, storage-unit contents now show real market names) | Current version: v3.8.0
...
- **Steam storage-unit import (TRANSITION-PLAN Phase 6)** — sessions 1 (GC connect + casket enumeration, v3.7.0, live-tested) and 2 (defindex → market_hash_name schema mapping, v3.8.0) shipped. **Session 3 remaining: merge-into-holdings flow** (reuse the v3.6.0 Steam-import dedupe/merge preview UX; the resolver already supplies `name`/`phase`/`wearName`/`kind` per item). Pro tier
